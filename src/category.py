...
class Category:
    ...
    расчёт суммарной стоимости, строковое представление категории и
    ...
    def add_product(self, product: object) -> None:
        ...
        Проверка выполняется через isinstance: она гарантирует, что переданный
        объект является экземпляром Product или любого его наследника
        (Smartphone, LawnGrass и т.п.).
    ...
    def average_price(self) -> float:
        ...
        Считает среднее арифметическое по self.price всех товаров.
        Если в категории нет товаров, возвращает 0.0 вместо деления на ноль.

        Реализация использует try/except для обработки ZeroDivisionError,
        как требовалось в задании.

        Returns:
            Средняя цена (float) или 0.0, если товаров нет.
        """
        try:
